    ...
    <v-dialog v-model="showModal" persistent max-width="600px">
        <v-card>
            ...
            <template v-if="selectedStation.isPlaying">
                <v-img :src="require('@/assets/audio-8777_256.gif')" height="200px"
                    @click="redirectToSite(selectedStation)"></v-img>
            </template>
            <template v-else-if="selectedStation.imageUrl">
                <v-img width="130" height="120" :src="selectedStation.imageUrl"
                    @click="redirectToSite(selectedStation)"></v-img>
            </template>
            <template v-else-if="selectedStation.favicon">
                <v-img width="130" height="120" :src="selectedStation.favicon"
                    @click="redirectToSite(selectedStation)"></v-img>
            </template>
            <template v-else>
                <v-icon @click="redirectToSite(selectedStation)">mdi-radio</v-icon>
                <!-- Use mdi-radio as default icon -->
            </template>
            <v-card-title>
            ...
            <v-card-text>
                <div v-if="selectedStation">
                    ...
                    <p v-if="selectedStation.site">
                        Site Link: <a class="site-link" :href="selectedStation.site" target="_blank">{{
                            selectedStation.site }}</a>
                    </p>
...
.v-dialog {
    z-index: 9999;
    /* Ensure modal is above Three.js scene */
}
</style>